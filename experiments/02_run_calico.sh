...
kind create cluster --config "$PROJECT_DIR/clusters/calico-cluster.yaml" --quiet
echo "[calico] Cluster ready"

# Step 2: Install CNI binaries
echo "[calico] Installing CNI binaries on nodes..."
...
echo "[calico] CNI binaries installed"

# Step 3: Install Calico
echo "[calico] Installing Calico (tigera operator)..."
kubectl create -f https://raw.githubusercontent.com/projectcalico/calico/v3.27.0/manifests/tigera-operator.yaml -q
sleep 20

echo "[calico] Applying Calico network config..."
cat <<EOFC | kubectl create -f - -q
...
"NAME:.metadata.name,CPU-REQ:.spec.containers[*].resources.requests.cpu,MEM-REQ:.spec.containers[*].resources.requests.memory"
  echo ""
  echo "Note: Calico ships with no fixed resource requests by design."
  echo "Typical observed usage: 100-250m CPU per node at idle."
  echo "calico-typha reduces API server load in larger clusters."
...
echo "[calico] All tests done. Results in $RESULTS_DIR"
