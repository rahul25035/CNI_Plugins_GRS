...
RESULTS_BASE = os.path.expanduser("~/cni-comparison/results")
CNIS = ["flannel", "calico", "cilium"]
COLORS = {"flannel": "#e05c5c", "calico": "#5b8dd9", "cilium": "#57b85a"}


# ---- Parse latency from ping output ----
# Looks for the "round-trip min/avg/max" summary line
def parse_latency(cni):
    ...
    if not os.path.exists(path):
        ...
        # Placeholder values based on typical CNI benchmarks - replace with real data
    ...
    # Match: round-trip min/avg/max = 0.045/0.120/0.198 ms
    # Also matches: rtt min/avg/max/mdev = 0.045/0.120/0.198/0.030 ms
...
def parse_bandwidth(cni):
    ...
    try:
        ...
        return round(bps / 1e9, 2)  # convert to Gbps
...
# ---- Chart 3: Radar chart (overall multi-dimension comparison) ----
# Scores are normalized 0-10 based on measured data + architecture knowledge
def plot_radar(latency, bandwidth):
    categories = ["Throughput", "Low Latency", "Simplicity", "Security Features", "Scalability"]
    N = len(categories)

    # Build normalized scores from measured data + known architecture properties
    # Throughput: normalized from bandwidth measurements
    bw_max = max(bandwidth.values()) if max(bandwidth.values()) > 0 else 1
    scores = {
        "flannel": [
            round(bandwidth["flannel"] / bw_max * 8, 1),   # throughput
            round((1 / latency["flannel"]["avg"]) * 0.1, 1) if latency["flannel"]["avg"] > 0 else 7,  # latency
            9.5,    # simplicity: Flannel is the simplest CNI
            3.0,    # security: no network policy support
            5.0,    # scalability: limited for large clusters
        ],
        "calico": [
            round(bandwidth["calico"] / bw_max * 8, 1),
            round((1 / latency["calico"]["avg"]) * 0.1, 1) if latency["calico"]["avg"] > 0 else 7,
            7.0,    # moderate complexity
            8.0,    # full network policy support via BGP
            8.5,    # scales well to large clusters
        ],
        "cilium": [
            round(bandwidth["cilium"] / bw_max * 8, 1),
            round((1 / latency["cilium"]["avg"]) * 0.1, 1) if latency["cilium"]["avg"] > 0 else 8,
            4.0,    # most complex: requires eBPF kernel knowledge
            9.5,    # best security: L7 policy, mTLS, identity-based
            9.5,    # best scalability: eBPF replaces iptables chains
        ],
    }
